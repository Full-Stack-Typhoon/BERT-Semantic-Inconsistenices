        ...
        for v in data:
            ...
            for i in range(1,len(v[1])):
                ...
                if word == 'location1':
                    ...
                    f.write(word)
            f.write("\t")
            f.write("what do you think of the ")
            if len(v[3])==1:
                f.write(v[3][0]+" ")
            else:
            ...
            for d in range(-1*context_size, context_size+1):
                if d != 0 and loc_index + d >= 0 and loc_index + d < len(v[1]):
                    ...
                    if v[2]=='LOCATION1':f.write('of location - 1 ? ')
                    if v[2]=='LOCATION2':f.write('of location - 2 ? ')
            f.write("\t")
            f.write(v[4]+"\n")


write_QA_M("train_QA_M.tsv", train)
write_QA_M("dev_QA_M.tsv", val)
write_QA_M("test_QA_M.tsv", test)
